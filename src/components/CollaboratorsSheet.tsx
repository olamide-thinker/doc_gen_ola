      ...
      {isOpen && (
        <>
          ...
          <motion.div
            ...
            className="fixed right-0 top-0 h-full w-full max-w-[380px] bg-card shadow-2xl z-[101] flex flex-col font-lexend border-l border-border"
          >
            {/* Header */}
            <div className="p-6 border-b border-border flex items-center justify-between">
              <div>
                <h2 className="text-lg font-black text-foreground tracking-tight">Collaborators</h2>
              ...
              {collaborators.map((col) => {
                ...
                return (
                  <div key={col.id} className="flex items-center gap-4 group">
                    {/* Avatar */}
                    <div 
                      className="w-12 h-12 rounded-2xl flex items-center justify-center text-white font-bold text-lg shadow-sm shrink-0 border-2 border-background ring-1 ring-border"
            ...
            <div className="p-6 border-t border-border bg-muted/50">
               <button
                onClick={() => logout()}
                className="w-full flex items-center justify-center gap-2 py-3 px-4 bg-card border border-border rounded-2xl text-xs font-bold text-muted-foreground hover:bg-muted hover:text-foreground transition-all shadow-sm"
               >
                 <LogOut size={14} />
                 Sign Out
               </button>
